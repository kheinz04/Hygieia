...
(function () {
    ...
    angular
        ...
        .directive('hideEmptyZero', function () {
            return {
                restrict: 'A',
                link: function (scope, element, attrs) {
                    attrs.$observe('hideEmptyZero', function () {
                        // Check if the inputs are all null, meaning the widget is empty (0 is NOT empty, could be a value)
                        var dataList = scope.$eval(attrs.hideEmptyZero);
                        var hideWidget = false;
                        var hideMsg;

                        dataList.forEach(function(item){
                            if (item.every(x => (!x && x != '0'))){
                                hideWidget = true;
                                hideMsg = "No data found.";
                            } else if (item.every(x => (x == '0'))){
                                hideWidget = true;
                                hideMsg = "No data found in the past <b>14</b> days.";
                            } else {
                                hideWidget = false;
                            }
                        });
                        ...
                        if (hideWidget) {
                            ...
                            if (noDataFound.length > 0) {
                                ...
                                element.parent().append('<div id="noDataMsg" class="row"><div class="col-md-12">'+hideMsg+'</div></div>');
                            }
                        } else {
